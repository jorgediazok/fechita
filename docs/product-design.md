...
- **DB**: MongoDB
- **Datos de partidos**: fuente intercambiable detrás de una interfaz `FixtureProvider`, elegida con `FIXTURE_SOURCE` (ver "Fuente de partidos" en `CLAUDE.md`), sincronizada por cron a la DB propia — nunca exponer una API externa directo a usuarios finales, así el costo escala con cantidad de partidos sincronizados, no con cantidad de usuarios. **Ningún free tier da la temporada argentina en curso**: API-Football (100 req/día) solo llega hasta 2024; TheSportsDB limita las listas a 1 resultado sin la key de supporter ($9/mes); football-data.org no cubre Argentina. Plan de pago más barato: TheSportsDB Premium $9/mes; API-Football Pro ~$19/mes. **Solución actual sin costo (decidida 2026-09-08)**: modo `replay` — se bajó la temporada 2024 completa y real de API-Football (free tier) a un JSON y se corre "al presente", una fecha por semana. Datos, equipos e IDs reales; la UI aclara que es "temporada de práctica". El día que se pague, se pasa a la temporada en vivo sin re-mapear nada.
