...
**Fase: loop central funcionando de punta a punta, con las capas 3 (ligas semanales, grupos privados) conectadas a un modelo real y auth real vía NextAuth.** El proyecto Next.js vive en [web/](web/) (`como-van-web`) — App Router + TypeScript + Tailwind, PWA básica, Mongoose con los modelos de las capas 1-3, conectado a un cluster real de MongoDB Atlas (`MONGODB_URI` en `web/.env.local`, no versionado). Ya anda: login real con NextAuth v5 (Google OAuth + email/password — ver "Auth" abajo), sync de partidos (`src/lib/sync.ts`) contra un proveedor simulado de API-Football (`API_FOOTBALL_MODE=mock`), carga de pronósticos con fichas 1-X-2 + bonus de resultado exacto opcional, cálculo de puntos 5/3/0, ligas semanales reales con categorías (`Primera D → C → B → Nacional → Primera División`), agrupamiento automático y ascenso/descenso al cerrar la semana (`src/lib/leagues.ts`, ver sección "Ligas semanales" abajo), y grupos privados de amigos con código de invitación (`src/lib/groups.ts`, ver sección "Grupos privados" abajo). Falta: conectar el proveedor real de API-Football (ya tenemos key, pero el free tier no sirve para temporada actual — ver más abajo), y el resto de las capas aditivas (insignias, trivia diaria).
